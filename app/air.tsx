...
export default function MonitorAirQuality() {
  const router = useRouter();
  const [airQuality, setAirQuality] = useState<number | null>(null);

  useEffect(() => {
    const airRef = ref(realtimeDb, 'log/gas');
    const unsubscribe = onValue(airRef, (snapshot) => {
      const val = snapshot.val();
      console.log('🌪 Firebase air quality (gas) value:', val);
      if (val !== null) setAirQuality(val);
    });
  ...
  const getQualityText = () => {
    if (airQuality === 1) return 'Healthy';
    if (airQuality === 0) return 'Unhealthy';
    return 'Loading...';
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        <Image source={require('@/assets/images/glow.air.png')} style={styles.glowImage} />
        <Text style={styles.readingLabel}>The reading of Air Quality is:-</Text>
        <Text style={styles.airValue}>{getQualityText()}</Text>
      </View>
